...
Здесь все элементы хранятся непосредственно в массиве. При коллизии используется пробирование - последовательный поиск свободной ячейки по определённому правилу. Удаление помечает элемент специальным маркером, чтобы не нарушать цепочки пробирования; окончательно записи удаляются только при перехешировании.
...
#fig("assets/generated/uml.png", caption: "UML-диаграмма классов", width: 80%) <fig:uml>
...
Для каждой стратегии использовался стандартный коэффициент загрузки. Измерялось среднее время операций при разных размерах таблицы.
#set table(inset: 2.5pt)
...
1. Страница по хеш-таблицам на Википедии - URL: https://en.wikipedia.org/wiki/Hash_table (дата обращения: 22.06.2026).
2. Документация по стандартной библиотеке C++ (cppreference.com). - URL: https://en.cppreference.com/ (дата обращения: 22.06.2026).
